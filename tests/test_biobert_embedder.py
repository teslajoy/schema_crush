"""test biobert embedder with real data."""
...
def test_embedder_initialization(biobert_embedder):
    """test that biobert embedder initializes correctly."""
    assert biobert_embedder.model is not None
    assert biobert_embedder.model_name == "dmis-lab/biobert-v1.1"


def test_single_embedding(biobert_embedder):
    """test generating single embedding."""
    embedding = biobert_embedder.embed("participant_id")
    assert embedding.shape[0] == 1
    assert embedding.shape[1] > 0  # has embedding dimension


def test_batch_embedding(biobert_embedder):
    """test generating multiple embeddings."""
    texts = ["participant_id", "primary_diagnosis", "tissue_type"]
    embeddings = biobert_embedder.embed(texts)
    assert embeddings.shape[0] == 3
    assert embeddings.shape[1] > 0


def test_similarity(biobert_embedder):
    """test similarity calculation between two terms."""
    sim = biobert_embedder.similarity("participant", "patient")
    assert 0.0 <= sim <= 1.0
    assert sim > 0.5  # these should be similar


def test_batch_similarity(biobert_embedder):
...
def test_with_loaded_data(biobert_embedder, loaded_data):
    """test biobert embedder with actual loaded csv data."""
    ...
    embeddings = biobert_embedder.embed(case_fields)
    ...
    sim_matrix = biobert_embedder.batch_similarity(case_fields, fhir_targets)
...
def test_three_tier_embeddings(biobert_embedder, loaded_data):
    ...
    entity_emb = biobert_embedder.embed_entity('case')
    patient_emb = biobert_embedder.embed('Patient')
    ...
    field_emb = biobert_embedder.embed_field('primary_diagnosis')
    condition_emb = biobert_embedder.embed('Condition.code')
    ...
    # tier 3: content embedding (name + values)
    diagnosis_values = case_df['primary_diagnosis'].tolist()
    content_emb = biobert_embedder.embed_content('primary_diagnosis', diagnosis_values)
    ...
    id_field_emb = biobert_embedder.embed_field('participant_id')
    id_content_emb = biobert_embedder.embed_content('participant_id', participant_values)
    patient_id_emb = biobert_embedder.embed('Patient.identifier')
